...
{
	int tempFd = *(int*)p;
	CSimpleBuffer	m_in_buf;	// RingBuffer
...
// 不同的command对应不同的对象，这样我们才能正确反序列化 通过id区分消息
void handle_login_request(CImPdu* pPdu) 
{
	IM::Login::IMLoginReq login;
	// ParseFromArray 自动生成的
    CHECK_PB_PARSE_MSG(login.ParseFromArray(pPdu->GetBodyData(), 
			pPdu->GetBodyLength()));
	std::string user_name = login.user_name();
    std::string password = login.password();
	std::string client_version = login.client_version();
	IM::BaseDefine::ClientType client_type = login.client_type();
