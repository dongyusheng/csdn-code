...
void CImPdu::WriteHeader()
{
	...
	CByteStream::WriteInt32(buf, GetLength());		// 整个PDU的数据长度
...
bool CImPdu::IsPduAvailable(uchar_t* buf, uint32_t len, uint32_t& pdu_len)
{
	...
	pdu_len = CByteStream::ReadUint32(buf); // header的首字段 length
	if (pdu_len > len)
	{
		//log("pdu_len=%d, len=%d\n", pdu_len, len);
		return false;
...
void CImPdu::SetPBMsg(const google::protobuf::MessageLite* msg)
{
    ...
    m_buf.Write(NULL, sizeof(PduHeader_t));	 // 1. pdu header
    ...
    m_buf.Write(szData, msg_size);		// 2. 写pb的数据
    delete []szData;
    WriteHeader();			// 
}
